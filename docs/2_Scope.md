...
| 9. SCT Operations | SADAR Context Token operations (Validate, Extract Claims, Append Segment, Verify Chain Integrity), segment structure, claims, chain semantics, and conformant cryptographic suites. |
...
**NOTE:** Where industry standards do not exist or, particularly at the operation level are not sufficiently granular, published API documentation can be used as the namespace.
...
**8. NFR Schema** companion document. The umbrella requirement is
...
defined normatively in the **8. NFR Schema** companion document.
...
- Where the SCT's originating user trust claim is `asserted`, the
  ...
  **10. Trust Models** companion document. These include asserter
...
in the **8. NFR Schema** companion document.
...
**8. NFR Schema** companion document.
...
the **8. NFR Schema** companion document.
...
##### §5.1.8.2 SADAR Context Token (SCT)

Authorization context propagated across trust boundaries. The SCT is
a JWS-inside-JWE token in which each chain segment is individually
signed by its issuer and encrypted to its intended recipient, so any
party holding the issuers' published keys can verify chain integrity
end-to-end even when it cannot decrypt segments addressed to others.
The SCT chain across multiple boundaries forms the cryptographically
attested record of how authorization context evolved through the
flow.

A conformant implementation SHALL support four operations over the
SCT chain:

| Operation | Purpose |
|-----------|---------|
| Validate | Verify each in-scope segment's JWS signature against the issuer's published key material and its structural conformance to the SCT format |
| Extract Claims | Return per-segment validity status and, where the caller is the JWE recipient, the decrypted claim set; segments that fail signature verification never return decrypted claims |
| Append Segment | Produce a new chain with one additional segment cryptographically bound to the prior tail, after validating the prior chain |
| Verify Chain Integrity | Verify end-to-end signature and parent-child binding integrity across all segments, operating on cleartext metadata without requiring decryption |

The complete SCT schema, segment structure, claim specifications,
operation semantics, error contract, and conformant cryptographic
suites are defined in the **9. SCT Operations** companion document.
...
**10. Trust Models** companion document.
...
fail-closed semantics are defined in the **10. Trust Models** companion
document.
...
the **12. searchAndInvoke Telemetry and Authentication** companion
document.
...
defined in the **11. Telemetry Record and Repatriation** companion
document. The SCT chain audit semantics are defined in the
**9. SCT Operations** companion document.
...
specification, and scope set are defined in the **12. searchAndInvoke
...
SADAR requires that conformant implementations make agent, tool,
and resource invocations **observable and attributable**, and that
they **support** governed telemetry repatriation across trust
boundaries. Those capabilities are supported, at a minimum, using
OpenTelemetry (OTel) baggage and repatriation.

**NOTE:** While SADAR supports OTel, OTel is NOT required to run
SADAR nor is it required to be present in a SADAR environment.

SADAR introduces the concept of OTel repatriation which must be
normatively implemented. In addition, registry components may
define a requirement or capability to support OTel repatriation as
part of the manifest definition.

Similarlly, SADAR must normatively support the following functions in OTel 
where OTel is used.

The following behaviors MUST occur in every conformant
implementation, independent of any particular telemetry technology:

- Invocations MUST be traceable, and the identifiers required for
  attribution and cross-step correlation — the transaction instance
  identifier and process context — MUST be generated and propagated
  across the steps of a flow
- Attribution sufficient for nonrepudiation MUST be preserved
  end-to-end; the signed SCT chain is the authoritative attribution
  and nonrepudiation record per §5.1.8, and observability data is
  corroborating rather than authoritative

**When OpenTelemetry is enabled for SADAR**, the following
mechanism-specific requirements apply:

- The process identifier MUST be carried in OTel baggage
- The accumulated risk score scalar MAY be carried in OTel baggage
  as a convenience value per §5.1.18; the authoritative per-step
  Adjustments are carried as signed claims in the SCT chain
  regardless of whether OTel is enabled
- W3C Trace Context propagation MUST be used for any invocation
  participating in cross-trust-boundary repatriation per §5.1.14.X;
  where baggage is used to carry SADAR context across a trust
  boundary, W3C Baggage propagation MUST be used
...
SADAR's observability design deliberately mirrors OpenTelemetry's
propagation architecture — its interval-bookend structure, ambient
in-process context propagation, and inject/extract wire model —
while remaining functionally independent of OTel. The architectural
relationship, and the requirement that SADAR maintain its own
context with no functional dependency on OTel, is specified in
**Section 8 of the SCT Propagation companion document**.
...
server-side validation are defined in the **10. Trust Models** and
**9. SCT Operations** companion documents.
...
A normative requirement that, **where SADAR observability and
repatriation layers are in use**, values appearing in multiple
...
observation. Parity applies only to the layers actually present in
a given deployment: where OTel is not enabled, parity is required
only among the layers in use (for example, SCT claims and Telemetry
Record fields). Parity instances include:
...
- Trust model claim across SCT and discovery-time negotiation
  result
- Risk score scalar carried in OTel against the scalar recomputed
  from the signed Adjustment chain in the SCT (applies only where
  the convenience scalar is carried in OTel baggage)
...
- Adjustments are emitted as structured objects carrying `delta`,
  `ttl_seconds`, `timestamp`, optional `decay`, and `reason` (an IRI
  that SHOULD be drawn from the SADAR-defined
  `urn:sadar:risk_reason:v1:*` enumeration; custom reasons use an
  organization-controlled namespace). Free-form rationale is not part
  of this signed claim; it is recorded in the OTel telemetry per the
  carriage rules below
- Each Adjustment — its `delta` and its SADAR-defined `reason` code —
  is carried as a signed, in-the-clear claim in the SCT chain segment
  of the step that emits it. The signed chain is the immutable,
  attributable record of which step contributed what, and why. Any
  confidential rationale beyond the reason code is recorded in the
  OTel telemetry, where it may be encrypted, and is never placed in
  the clear SCT claim
- When OTel is enabled, the accumulated scalar MAY be carried in
  OTel baggage as a convenience value. Because accumulation is
  deterministic (below), any authorized
  party may recompute the scalar by walking the signed chain; the OTel
  value is therefore a cache, provable at will against the SCT and not
  itself a trusted input
- An Adjustment is the immutable contribution of the participant
  that emitted it; a conformant implementation SHALL accept and
  propagate Adjustments without dropping, reordering, or modifying
  them, SHALL preserve unknown reason IRIs, SHALL reject malformed
  Adjustments, and SHALL NOT alter an Adjustment emitted by another
  participant
- A conformant implementation SHALL compute the convenience scalar by
  a single defined accumulation — the sum of currently-active
  (non-expired) Adjustment deltas, floor-clamped at 0.0 and
  ceiling-clamped at 1.0. This accumulation is deterministic: the
  same active Adjustment set yields the same scalar in every
  conformant implementation. SADAR performs it as a convenience, not
  as a risk judgment
- The interpretation of the scalar — any domain-specific weighting,
  risk-sensitivity calibration, threshold setting, or policy-driven
  recomputation — is performed by the consuming enforcement layer and
  is out of scope per §5.2.8
- A consuming participant, including an enforcement layer, MAY emit
  its own Adjustment following its own policy evaluation or
  mitigation; doing so re-enters the same simple accumulation and the
  scalar is recomputed. Such a participant SHALL NOT modify any other
  participant's Adjustment
- Intermediate summation values may fall below 0.0 or above 1.0; the
  reported scalar is clamped to the `[0.0, 1.0]` interval
- The Telemetry Record persists the scalar and per-step Adjustment
  detail at finalize for at-rest audit; the signed SCT chain remains
  the authoritative, attributable record of the Adjustments themselves
  per §5.1.8.2
...
and impact_score semantics are defined in the **13. Risk Score
...
- **Observability and Repatriation** — observability and
  attribution per §5.1.14; where OTel is enabled, OTel
  instrumentation as defined in §5.1.14, including the process
  identifier, required baggage fields, and provenance attribution
  on every span; and support for OTel-based telemetry repatriation
  where bilateral matching at discovery requires it
...
The IP licensing grant extends to the normative observability and
repatriation requirements defined in §5.1.14, including the
Telemetry Record schema, span structure with provenance
attribution, and Telemetry Repatriation. How an implementation
consumes, processes, routes, stores, monitors, or acts upon that
telemetry data beyond those normative requirements is out of scope.
...
Adjustment data model, propagation rules, accept-and-propagate
behavior, and the deterministic simple accumulation defined in
§5.1.18. The interpretation and weighting applied to the accumulated
scalar, the risk-sensitivity calibration and policy-driven
recomputation an enforcement layer may perform, the intervention
thresholds at which an implementation acts on accumulated risk, and
the specific mitigation techniques an agent or guardrail applies are
all out of scope. Equally out of scope are
the detection, scoring, and analytical methods by which
a participant generates an Adjustment — including drift-versus-baseline
checks, attestation gating, bias or hallucination detection, and other
implementer-proprietary techniques — as distinct from the normative
obligation to accept and propagate Adjustments faithfully.
